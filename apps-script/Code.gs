...
function doGet(e) {
  try {
    ...
    if (!data.name && !data.email) {
      return jsonResponse({ status: "WAFIL API çalışıyor." });
    }

    Logger.log("Başvuru: " + JSON.stringify(data));

    sendNotification(data);
    sendAutoReply(data);

    Logger.log("Emailler gönderildi.");
...
function sendNotification(data) {
  ...
  var htmlBody = [
    ...
    '<a href="mailto:' + data.email + '" style="display:inline-block;background:#0b2e1f;color:#ffd21f;padding:10px 22px;border-radius:8px;text-decoration:none;font-weight:bold">Yanıtla</a>',
    '</div>',
    '</div>',
    '</div>',
  ].join("");
